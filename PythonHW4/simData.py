...
tx3 = stats.multinomial.rvs(1, p=[0.5, 0.25, 0.25], size=N)
tx_strings = ['Placebo', 'tx1', 'tx2']
tx = np.array([tx_strings[t.tolist().index(1)] for t in tx3])

female = stats.binom.rvs(size=N, n=1, p=0.5)

age = stats.norm.rvs(size=N, loc=40, scale=20).round()

err = stats.norm.rvs(size=N, loc=0, scale=3)

score = 45.0 + -0.2*(age - 40.0) + 0.01*(age - 40.0)**2 + \
        8.5*(tx == 'tx1') + 10*female + 8.5*(tx == 'tx2')*female + err
score = score.round(2)


def randTx(tx):
    """ randomize the form of tx """
    if random.uniform(0.0, 1.0) > 0.3:
        return tx.lower()
    return tx[0].upper() + tx[1:]


tx = pd.Series(map(randTx, tx))
hw4 = pd.DataFrame({'age': age, 'tx': tx, 'female': female, 'score': score})
hw4.to_csv("hw4.csv", index=False)
